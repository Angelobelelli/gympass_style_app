...
describe('Check-in Use Case', () => {
  ...
  it('should be able to check in', async () => {
    const { checkIn } = await sut.execute({
      userId: 'user-01',
      gymId: 'gym-01',
      userLatitude: -27.20925,
      userLongitude: -40.177162,
    })
  ...
  it('should not be able to check in on distant gym', async () => {
    gymsRepository.items.push({
      ...
      description: '',
      phone: '',
      latitude: new Decimal(-20.126084),
      longitude: new Decimal(-40.184955),
    })
    await expect(() =>
      sut.execute({
        userId: 'user-01',
        gymId: 'gym-02',
        userLatitude: -20.116454,
        userLongitude: -41.177162,
      }),
